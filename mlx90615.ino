...
//#define USE_DUE
#define USE_MEGA2560
#define DTPOINT_TOTAL 120 //1200 //120 //1300  //1780  //2400   //do not use algebra here
...
#define READ_INTERVAL 10    //5s  //6s    //multilply RUN_DELAY_MILLI
#define RUN_DELAY_MILLI 500  //200ms //1000ms 
...
void setup()
{
  ...
  */
  ...
  if (c == 0x68 || c==0x98) // WHO_AM_I should always be 0x68   //or 0x98 for icm-20689
  {  
    ...
    bMPU6050Available=true;
   }
   else
   {
    Serial.print("Could not connect to MPU6050: 0x");
    Serial.println(c, HEX);
    bMPU6050Available=false;
    //while(1) ; // Loop forever if communication doesn't happen
   }

  }

  Serial.print("sizeof(long) is ");
...
void loop()
{
  ...
  unsigned char coutbuf2[4 + (SENSOR_TOTAL+EXTRA_VIRTUAL_SENSOR) * 2 * 2 + 1];   //including time,data,gyro value 
  short *pData;
  double tPt100;
  int iRTDStatus=0;
  ...
  if (iruntimes%READ_INTERVAL==0) {
    ...
    rtd.read_all( );   //这个不能提到setup()里，否则第一次上电会无法访问RTD，需要手工重启
    iRTDStatus=rtd.status();
    if( iRTDStatus == 0 )
    {
    ...
    else 
    {
      ...
      Serial.print( iRTDStatus );
      Serial.print( ": " );
      if( iRTDStatus & MAX31865_FAULT_HIGH_THRESHOLD )
      {
        Serial.println( "RTD high threshold exceeded" );
      }
      else if( iRTDStatus & MAX31865_FAULT_LOW_THRESHOLD )
      {
        Serial.println( "RTD low threshold exceeded" );
      }
      else if( iRTDStatus & MAX31865_FAULT_REFIN )
      {
        Serial.println( "REFIN- > 0.85 x V_BIAS" );
      }
      else if( iRTDStatus & MAX31865_FAULT_REFIN_FORCE )
      {
        Serial.println( "REFIN- < 0.85 x V_BIAS, FORCE- open" );
      }
      else if( iRTDStatus & MAX31865_FAULT_RTDIN_FORCE )
      {
        Serial.println( "RTDIN- < 0.85 x V_BIAS, FORCE- open" );
      }
      else if( iRTDStatus & MAX31865_FAULT_VOLTAGE )
      {
  ...
  if (cinbuf1[0] == 0xaa && cinbuf1[1] == 0xaa)
  {
    ...
    if (cinbuf1[2] == 2) {
      ...
      for (i = ireadpointer; i < iend; i++) {
        ...
        coutbuf2[sizeof(long)+(SENSOR_TOTAL+EXTRA_VIRTUAL_SENSOR)*2*2]=(unsigned char)igyroVal[ip];
        
        crc = crc8Msb(7,(unsigned byte*)coutbuf2, sizeof(long) + (SENSOR_TOTAL+EXTRA_VIRTUAL_SENSOR) * 2 * 2 + 1);

        /*for (int iveri=0;iveri<sizeof(long) + (SENSOR_TOTAL+EXTRA_VIRTUAL_SENSOR) * 2 * 2 + 1;iveri++) {
          char cbufveri[16];
          sprintf(cbufveri,"%02x",coutbuf2[iveri]);
          Serial.print(cbufveri);
          Serial.print(" ");
        }
        Serial.println(crc);*/
...
//20220105 把rtd.read_all放前面 修改为间隔500毫秒 5秒读一次温度，另外开头先检查有没有MPU6050，另外rtd.status数值缓存一下，防止那里浪费时间
//20220112 把rtd.read_all放回loop（否则热电阻第一次上电读不到需要重启）
